...
export const getUserTransactionsByType = async (
  ...
  try {
    const transactions = await TransactionModel.getTransactionsByType(
      id,
...
export const getUserTransactionsByStatus = async (
  req: Request,
  res: Response
) => {
  const { id } = req.params;
  ...
  try {
    const transactions = await TransactionModel.getTransactionsByStatus(
      id,
